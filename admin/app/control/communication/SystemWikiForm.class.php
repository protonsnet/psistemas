...
class SystemWikiForm extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $img = new TEntry('img');
        $categoryid    = new TDBCombo('categoryid','communication','Category','id','description');
        //$url = new TEntry('url');
        ...
        $row2 = $this->form->addFields([new TLabel(_t("Image") . ':', '#ff0000', '14px', null, '100%'),$img]);
        $row2->layout = [' col-sm-12'];

        $row3 = $this->form->addFields([new TLabel(_t("Category") . ':', '#ff0000', '14px', null, '100%'),$categoryid]);
